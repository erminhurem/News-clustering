...
def get_or_create_sources_for_news(news):
    # Ovdje definirajte logiku za identificiranje izvora za odreÄ‘enu vijest
    # Na primjer, moĹľete koristiti kljuÄŤne rijeÄŤi iz opisa vijesti

    # Pretpostavimo da imate listu kljuÄŤnih rijeÄŤi koje odgovaraju razliÄŤitim izvorima
    keywords_to_sources = {
        'kljucna rijec 1': 'Izvor 1',
        'kljucna rijec 2': 'Izvor 2',
        # Dodajte ostale parove kljuÄŤna rijeÄŤ - izvor
    }
...
def news_at_10(request):
    ...
    # Filtriramo vijesti objavljene izmeÄ‘u 10:00 i 10:59
...
# PomoÄ‡na funkcija za dobijanje relativnog vremena
...
def clean_description(description):
    ...
    # Uklanjanje HTML tagova koristeÄ‡i BeautifulSoup
    ...
    for img_tag in soup.find_all('img'):
      ...
      # Uklanjanje samo <p> tagova, ali zadrĹľavanje teksta unutar
    for p_tag in soup.find_all('p'):
        p_tag.unwrap()
    
    # Dobijanje ÄŤistog teksta
...
def index(request):
    ...
    # IzraÄŤunavanje TF-IDF vektora za trenutnu vijest
    ...
    for category in categories:
        ...
        for item in news_items:
            ...
            # IzraÄŤunajte povezane vijesti za svaku vijest
            current_tfidf = vectorizer.transform(
...
# poÄŤetak koda vezano za rubriku Sport
def sport(request):
...
# poÄŤetak koda vezano za rubriku Magazin
...
def najnovije_vijesti(request):
    ...
    kategorije = [
        ...
        'Fudbal', 'KoĹˇarka', 'Tenis', 'Ostalo', 'Automobili / Motori', 'Vijesti'
    ]
...
def extract_images(entry):
    ...
    # Ekstrakcija iz 'description' ako sadrĹľi 'img' tagove
...
def parse_date(date_str):
    try:
        ...
        logger.error(f"GreĹˇka prilikom parsiranja datuma: {e}")
        return None
    ...
    for feed_url in feeds:
        ...
        for entry in feed.entries:
            if not Headlines.objects.filter(link=entry.link).exists():
                ...
                description = clean_description(entry.description)  # Cišcenje opisa
...
def categorize_news(url):
    # Mapiranje kljuÄŤnih rijeÄŤi na kategorije
    kategorije = {
    ...
    # Default kategorija ako se ne pronaÄ‘e niĹˇta
...
def company_directory(request):
    ...
    # Dobijanje broja firmi po opĹˇtinama
    companies_count_by_city = create_company_directory_adjusted(file_paths)  # Ispravljeno ime varijable

    context = {
        'companies_count_by_city': companies_count_by_city,  # Ispravljeno ime kljuÄŤa u kontekstu
...
def city_companies(request, city_name):
    # Pretpostavljamo da city_name dolazi iz URL-a i koristit Ä‡emo ga za filtriranje podataka
    file_paths = [
        settings.BASE_DIR / 'static' / 'Baza 2000.xlsx',
        settings.BASE_DIR / 'static' / 'Baza 2001.xlsx',
        settings.BASE_DIR / 'static' / 'Baza 2003.xlsx',
    ...
    for fp in file_paths:
        # Ovdje pretpostavljamo da fajl sadrĹľi kolonu 'OpĹˇtina' koja sadrĹľi nazive opĹˇtina
    ...
    # Filtriranje podataka za odabranu opĹˇtinu
    city_companies_data = all_data[all_data['OpĹˇtina'].str.contains(city_name_str, na=False)]
...
def last_cron_run(request):
    try:
        ...
        last_updated = None  # U slučaju greške postaviti na None ili trenutno vrijeme
