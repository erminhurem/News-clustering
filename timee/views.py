...
import traceback
from timee.friendly_name import get_friendly_source_name
    ...
    for keyword, source_name in keywords_to_sources.items():
        if keyword in news.description:
            source, created = Source.objects.get_or_create(
                name=source_name, link='URL za izvor')
...
def index(request):
    ...
    related_news_ids = [all_news[i.item(
    )].id for i in related_articles_indices if all_news[i.item()].id != news.id]
    # Ovo će postaviti povezane vijesti
    news.related_news.set(related_news_ids)

    # psotavljanje dobijenih izvora
    sources = get_or_create_sources_for_news(news)
    news.other_sources.set(sources)

    categories = ['Ekonomija', 'BiH', 'Balkan', 'Svijet',
                  'Hronika', 'Sarajevo', 'Kultura', 'Scena', 'Sport', 'Magazin']
    news_by_category = {}
    for category in categories:
        news_items = Headlines.objects.filter(category=category).order_by(
            '-published_date').prefetch_related('related_news')[:3]
        for item in news_items:
            ...
            related_news_ids = [all_news[i.item(
            )].id for i in related_articles_indices if all_news[i.item()].id != item.id]
...
def bih_category(request):
    news_items = Headlines.objects.filter(
        category='BiH').order_by('-published_date')
    vectorizer = TfidfVectorizer()
    last_updated = LastFetch.get_last_update_time()
    ...
    # Dohvaćanje povezanih vijesti iz baze podataka
    related_news_ids = [news_items[i.item(
    )].id for i in related_articles_indices if news_items[i.item()].id != news.id]
    # Ovo će postaviti povezane vijesti
    news.related_news.set(related_news_ids)
...
def ekonomija_category(request):
    news_items = Headlines.objects.filter(
        category='Ekonomija').order_by('-published_date')
    vectorizer = TfidfVectorizer()
    last_updated = LastFetch.get_last_update_time()
...
def balkan_category(request):
    news_items = Headlines.objects.filter(
        category='Balkan').order_by('-published_date')
    last_updated = LastFetch.get_last_update_time()
...
def svijet_category(request):
    news_items = Headlines.objects.filter(
        category='Svijet').order_by('-published_date')
    vectorizer = TfidfVectorizer()
    last_updated = LastFetch.get_last_update_time()
...
def sarajevo_category(request):
    news_items = Headlines.objects.filter(
        category='Sarajevo').order_by('-published_date')
    vectorizer = TfidfVectorizer()
    last_updated = LastFetch.get_last_update_time()
...
def hronika_category(request):
    news_items = Headlines.objects.filter(
        category='Hronika').order_by('-published_date')
    vectorizer = TfidfVectorizer()
    last_updated = LastFetch.get_last_update_time()
...
def kultura_category(request):
    news_items = Headlines.objects.filter(
        category='Kultura').order_by('-published_date')
    vectorizer = TfidfVectorizer()
    last_updated = LastFetch.get_last_update_time()
...
def scena_category(request):
    news_items = Headlines.objects.filter(
        category='Scena').order_by('-published_date')
    vectorizer = TfidfVectorizer()
    last_updated = LastFetch.get_last_update_time()
...
def sport(request):
    latest_news_s = Headlines.objects.filter(
        category="Sport").order_by('-published_date')[:3]
    vectorizer = TfidfVectorizer()
    last_updated = LastFetch.get_last_update_time()
    ...
    related_news_ids = [latest_news_s[i.item(
    )].id for i in related_articles_indices if latest_news_s[i.item()].id != news.id]
    # Ovo ce postaviti povezane vijesti
    news.related_news.set(related_news_ids)
    ...
    for category in categories:
        ...
        for item in news_items:
            ...
            related_articles_indices = cosine_similarities[0].argsort()[:-5:-1]
            related_news_ids = [latest_news_s[i.item(
            )].id for i in related_articles_indices if latest_news_s[i.item()].id != item.id]
            item.related_news.set(related_news_ids)
        news_by_category[category] = news_items
...
def fudbal_category(request):
    news_items = Headlines.objects.filter(
        category='Fudbal').order_by('-published_date')
    vectorizer = TfidfVectorizer()
    last_updated = LastFetch.get_last_update_time()
...
def kosarka_category(request):
    news_items = Headlines.objects.filter(
        category='Kosarka').order_by('-published_date')
    vectorizer = TfidfVectorizer()
    last_updated = LastFetch.get_last_update_time()
...
def tenis_category(request):
    news_items = Headlines.objects.filter(
        category='Tenis').order_by('-published_date')
    vectorizer = TfidfVectorizer()
    last_updated = LastFetch.get_last_update_time()
...
def ostalo_category(request):
    news_items = Headlines.objects.filter(
        category='Ostalo').order_by('-published_date')
    vectorizer = TfidfVectorizer()
    last_updated = LastFetch.get_last_update_time()
...
def magazin(request):
    vectorizer = TfidfVectorizer()
    latest_news_m = Headlines.objects.filter(
        category="Magazin").order_by('-published_date')[:3]
    last_updated = LastFetch.get_last_update_time()
    ...
    # Dohvacanje povezanih vijesti iz baze podataka
    related_news_ids = [latest_news_m[i.item(
    )].id for i in related_articles_indices if latest_news_m[i.item()].id != news.id]
    # Ovo ce postaviti povezane vijesti
    news.related_news.set(related_news_ids)
    ...
    categories = ['Zabava', 'Automobili', 'Tehnologija',
                  'Lifestyle', 'Hrana', 'Intima', 'VIJESTI', 'SPORT']
    news_by_category = {}
    for category in categories:
        news_items = Headlines.objects.filter(
            category=category).order_by('-published_date')[:3]
        for item in news_items:
            ...
            related_articles_indices = cosine_similarities[0].argsort()[:-5:-1]
            related_news_ids = [latest_news_m[i.item(
            )].id for i in related_articles_indices if latest_news_m[i.item()].id != item.id]
            item.related_news.set(related_news_ids)
        news_by_category[category] = news_items
...
def zabava_category(request):
    news_items = Headlines.objects.filter(
        category='Zabava').order_by('-published_date')
    vectorizer = TfidfVectorizer()
    last_updated = LastFetch.get_last_update_time()
...
def automobili_category(request):
    news_items = Headlines.objects.filter(
        category='Automobili').order_by('-published_date')
    vectorizer = TfidfVectorizer()
    last_updated = LastFetch.get_last_update_time()
...
def tehnologija_category(request):
    news_items = Headlines.objects.filter(
        category='Tehnologija').order_by('-published_date')
    vectorizer = TfidfVectorizer()
    last_updated = LastFetch.get_last_update_time()
...
def lifestyle_category(request):
    news_items = Headlines.objects.filter(
        category='Lifestyle').order_by('-published_date')
    vectorizer = TfidfVectorizer()
    last_updated = LastFetch.get_last_update_time()
...
def hrana_category(request):
    news_items = Headlines.objects.filter(
        category='Hrana').order_by('-published_date')
    vectorizer = TfidfVectorizer()
    last_updated = LastFetch.get_last_update_time()
...
def intima_category(request):
    news_items = Headlines.objects.filter(
        category='Intima').order_by('-published_date')
    vectorizer = TfidfVectorizer()
    last_updated = LastFetch.get_last_update_time()
...
def najnovije_vijesti(request):
    ...
    if topic == 'all':
        ...
        latest_news = Headlines.objects.filter(
            category=topic).order_by('-published_date')[:10]
    ...
    # Dohvacanje povezanih vijesti iz baze podataka
    related_news_ids = [latest_news[i.item(
    )].id for i in related_articles_indices if latest_news[i.item()].id != news.id]
    # Ovo ce postaviti povezane vijesti
    news.related_news.set(related_news_ids)
...
def extract_images(entry):
    ...
    if 'media_content' in entry:
        ...
        if media_content:
            images.extend([content['url']
                          for content in media_content if 'url' in content])
    ...
    if 'enclosures' in entry:
        ...
        images.extend([enclosure['url']
                      for enclosure in enclosures if 'url' in enclosure])
...
def fetch_news():
    ...
    feeds = [
        ...
        'https://www.tanjug.rs/rss/svet',
        'https://www.vijesti.me/rss',
        'https://www.pobjeda.me/',
        'https://crnogorskiportal.me/',
        'https://mondo.me/rss/629/Naslovna',
        'https://rtcg.me/rss/',
        'https://novabh.tv/feed/',
        'https://sop.ba/feed/',
        'https://dobojski.info/',
        'https://granicedoboja.info/feed/',
        'https://www.brckodanas.com/feed/',
        'https://zepce.ba/feed/',
        'https://www.federalna.ba/',
        'https://grad-busovaca.com/feed/',
        'https://www.ljportal.com/feed/',
        'https://www.direkt-portal.com/feed/',
        'https://gorazdeportal.com/feed/',
        'https://www.visocki.info/feed/',
        'https://magazinplus.eu/feed/',
        'https://visoko.ba/feed/',
        'https://gracanicki.ba/feed/',
        'https://hayat.ba/feed/',
        'https://www.fokus.ba/feed/',
        'https://bhportal.ba/feed/',
        'https://www.vecernji.ba/feeds/latest',
        'https://istraga.ba/feed/',
        'https://bikemagazin.info/feed/',
        'https://www.profitiraj.ba/feed/',
        'https://yep.ba/feed/',
        'https://biznis.ba/rss',
        'https://www.carlander.ba/feed/',
        'https://auto.ba/feed/',
        'https://lifestyle.ba/feed/',
        'https://www.ljepotaizdravlje.ba/feed',
        'https://reprezentacija.ba/feed',
        'https://sportske.ba/feed/',
        'https://senzacija.ba/feed/',
        'https://ekskluziva.ba/',
        'https://zdraviportal.ba/feed/',
        'https://senzacija.ba/feed/',
        'https://pogled.ba/rss',
        'https://bosnainfo.ba/feed/',
        'https://caportal.net/feed/',
        'https://sirokibrijeg.info/feed/',
        'https://www.jabuka.tv/feed/',
        'https://novasloboda.ba/feed/',
        'https://mostarski.info/feed/',
        'https://blmojgrad.com/feed/',
        'https://www.bl-portal.com/feed/',
        'https://tip.ba/feed/',
        'https://zepce.live/feed/',
        'https://www.zenit.ba/feed/',
        'https://www.072info.com/feed/',
        'https://www.zenicablog.com/feed/',
        'https://cazin.ba/feed/',
        'https://www.hercegovina.info/rss?categoryId=1',
        'https://bosnainfo.ba/feed/',
        'https://ilijas.net/feed/',
        'https://detektor.ba/feed/',
        'https://tuzlalive.ba/feed/',
        'https://www.capital.ba/feed/',
        'https://pressmediabih.com/feed/',
        'https://poskok.info/feed/',
        'https://www.biznisinfo.ba/feed/',
        'https://politicki.ba/feed',
        'https://thebosniatimes.ba/feed/',
        'https://krajina.ba/feed/',
        'https://www.akta.ba/rss',
        'https://centralna.ba/feed/',
        'https://www.otisak.ba/feed/'
        'https://zenicainfo.ba/feed',
        'https://www.times.ba/feed/',
        'https://azra.ba/feed/',
        'https://jajce-online.com/feed/',
        'https://izdvojeno.ba/feed/',
        'https://bljesak.info/rss',
        'https://tuzlanski.ba/feed/',
        'https://crna-hronika.info/feed',
        'https://p-portal.net/feed/',
        'https://www.portalnovosti.com/',
        'https://sportsport.ba/feed',
        ...
        for entry in feed.entries:
            if not Headlines.objects.filter(link=entry.link).exists():
                published_date = parse_date(
                    entry.published) if entry.published else None

                # Koristi definisanu funkciju za ekstrakciju slika
                image_urls = extract_images(entry)
                # Ovdje dodajemo kategoriju
                category = categorize_news(entry.link)
                description = clean_description(
                    entry.description)  # Ciscenje opisa
...
def news_archive(request):
    ...
    if date_from and date_to:
        news_items = Headlines.objects.filter(
            published_date__range=[date_from, date_to])
    else:
...
def news_archive(request):
    ...
    news_list = [{'title': news.title, 'description': news.description}
                 for news in news_items]
...
def opstine_view(request):
    last_updated = LastFetch.get_last_update_time()
    opstine = Firme.objects.values('opstina').annotate(
        broj_firmi=Count('id_broj')).order_by('opstina')
...
def search_news(request):
    ...
    news_results = Headlines.objects.filter(
        title__icontains=query) if query else Headlines.objects.all()
    last_updated = Headlines.objects.latest('published_date').published_date

    # Paginacija
    # 10 vijesti po stranici, prilagodite prema potrebi
    paginator = Paginator(news_results, 10)
...
def uslovi_indeksiranja(request):

    return render(request,"uslovi_indeksiranja.html")

def uslovi_koristenja(request):

    return render(request,"uslovi_koristenja.html")

def politika_privatnosti(request):

    return render(request,"politika_privatnosti.html")

def time_site(request):
